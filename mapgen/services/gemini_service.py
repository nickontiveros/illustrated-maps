...
class GeminiService:
    ...
    STYLE_PROMPTS = {
        "base_map": (
            ...
            "aesthetic with warm, saturated colors. The map should have an isometric "
            "aerial view perspective. Maintain the exact layout of roads, buildings, "
            "water features, and parks. Roads should be clearly visible as white or "
            "cream colored paths. Buildings should have a warm tan/beige color with "
            "subtle shadows. Water should be a pleasant blue. Parks and grass should "
            "be vibrant green. Create clean edges suitable for seamless tiling with "
    ...
    def generate_base_tile(
        ...
        Args:
            reference_image: Base map render (OSM styled image)
        ...
        image_bytes = self._image_to_bytes(reference_image)
    ...
    def stylize_landmark(
        ...
        photo_bytes = self._image_to_bytes(photo)
        ...
        if style_reference:
            style_bytes = self._image_to_bytes(style_reference)
            response = self.client.generate_content(
                [
                    {"mime_type": "image/png", "data": photo_bytes},
                    {"mime_type": "image/png", "data": style_bytes},
                    f"Transform the first image to match the illustrated style of the second image. {prompt}",
                ],
                generation_config=generation_config,
            )
        else:
    ...
    def inpaint_seam(
        ...
        if orientation == "horizontal":
            ...
            combined = Image.new("RGBA", (w, h * 2))
            combined.paste(region_a, (0, 0))
            combined.paste(region_b, (0, h))

        combined_bytes = self._image_to_bytes(combined)
        ...
        response = self.client.generate_content(
            [
                {"mime_type": "image/png", "data": combined_bytes},
                prompt,
            ],
            generation_config=generation_config,
        )
